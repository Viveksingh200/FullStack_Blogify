import { CircleUserRound, RefreshCcw, Rss, Search } from "lucide-react";
    ...
    <div className="flex md:w-9/12 mx-auto flex-col gap-4 my-10 p-3">
      ...
      <div className="w-full flex flex-col md:flex-row gap-6 items-start justify-center">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span 
            onClick={fetch}
            className={`flex items-center gap-2 border  rounded-md p-2 px-6 cursor-pointer ${!loading ? "border-gray-300 text-gray-600" : "border-amber-300 text-amber-600 bg-amber-50"}`}
          ><RefreshCcw className={`${loading && "animate-spin"}`}/></span>
        </div>
      </div>
            ...
            {blogs && blogs.length !== 0
              ? blogs.map((blog) => (
                  ...
                  >
                    ...
                    <div className="flex items-center w-full justify-between border-t pt-2 border-gray-200">
                      <span className="font-semibold italic">Author: Myself</span>
                      <span className={`${blog.visibility === "public" ? "text-amber-400" : "text-red-500"}`}>
            ...
            {publicBlogs
              ? publicBlogs.map((blog) => (
                  ...
                  >
                    ...
                    <div className="flex items-center w-full justify-between border-t pt-2 border-gray-200">
                      <span className="font-semibold italic">Author: {blog?.author.name || "Unknown"}</span>
                      <span>
                        {new Date().toDateString(blog?.createdAt)}
                      </span>
                    </div>
                  </div>
                ))
